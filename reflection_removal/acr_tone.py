"""Adobe Camera Raw baseline tone rendering, transliterated from the DNG SDK 1.7.1.

Two pieces, both from dng_sdk/source:

  * ``_ACR3`` is ``dng_tone_curve_acr3_default::kTable`` (dng_render.cpp), the
    tone curve ``dng_render`` uses by default (dng_render.cpp:2171). 1025 samples
    of a monotone S-curve on linear [0, 1].
  * ``baseline_rgb_tone`` is ``RefBaselineRGBTone`` (dng_reference.cpp:1871):
    the curve is applied to the max and min channels only, and the middle channel
    is interpolated so that (mid - min) / (max - min) is preserved. That is what
...
# dng_tone_curve_acr3_default::kTable -- DNG SDK 1.7.1, dng_render.cpp
_ACR3 = np.array([
    ...
    1.00000,], dtype=np.float64)

_X = np.linspace(0.0, 1.0, _ACR3.size)


def tone_curve(x):
    """The scalar ACR default curve on linear [0, 1]. Values outside are clamped."""
    return np.interp(np.clip(x, 0.0, 1.0), _X, _ACR3)


def tone_curve_inverse(y):
    """Inverse of :func:`tone_curve` (the table is monotone increasing)."""
    return np.interp(np.clip(y, 0.0, 1.0), _ACR3, _X)


def baseline_rgb_tone(rgb):
    """RefBaselineRGBTone: apply the curve to max/min, interpolate the middle.

    rgb: (..., 3) linear sRGB in [0, 1]. Preserves hue the way ACR does.
    """
    x = np.clip(np.asarray(rgb, np.float32), 0.0, 1.0)
    order = np.argsort(x, axis=-1)                       # ascending: min, mid, max
    lo = np.take_along_axis(x, order[..., 0:1], -1)[..., 0]
    md = np.take_along_axis(x, order[..., 1:2], -1)[..., 0]
    hi = np.take_along_axis(x, order[..., 2:3], -1)[..., 0]

    HI = tone_curve(hi)
    LO = tone_curve(lo)
    span = hi - lo
    MD = np.where(span > 0, LO + (HI - LO) * (md - lo) / np.maximum(span, 1e-12), HI)

    out = np.empty_like(x)
    np.put_along_axis(out, order[..., 0:1], LO[..., None].astype(np.float32), -1)
    np.put_along_axis(out, order[..., 1:2], MD[..., None].astype(np.float32), -1)
    np.put_along_axis(out, order[..., 2:3], HI[..., None].astype(np.float32), -1)
    return out
...
def finish(lin_srgb, ev=0.0, tone=True):
    """Linear sRGB -> 8-bit sRGB, the ACR baseline way.

    Drop-in replacement for isp(). Pass tone=False to get the gamma-only render
    (the paper's own output format) so the two datasets differ in this step alone.
    """
    x = np.clip(np.asarray(lin_srgb, np.float32), 0.0, None) * (2.0 ** ev)
    x = np.clip(x, 0.0, 1.0)
    if tone:
        x = baseline_rgb_tone(x)
    return (srgb_encode(x) * 255 + 0.5).astype(np.uint8)


if __name__ == "__main__":
    print(f"table: {_ACR3.size} entries, monotone: {bool(np.all(np.diff(_ACR3) >= 0))}")
    for v in (0.02, 0.05, 0.1329, 0.18, 0.5, 0.9):
        print(f"  {v:.4f} -> {float(tone_curve(v)):.4f}   (x{float(tone_curve(v))/v:.2f})")
    g = np.random.default_rng(0).random((4, 4, 3)).astype(np.float32)
    t = baseline_rgb_tone(g)
    print("hue preserved (max/mid/min order kept):",
          bool(np.all(np.argsort(g, -1) == np.argsort(t, -1))))
    print("round trip max err:", float(np.abs(tone_curve_inverse(tone_curve(g)) - g).max()))
